...
static NSString *const kSchemaKeywordAdditionalProperties =
    @"additionalProperties";
static NSString *const kSchemaKeywordPatternProperties = @"patternProperties";

- (instancetype)
    initWithPropertySchemas:(NSDictionary *)propertySchemas
 additionalPropertiesSchema:(VVJSONSchema *)additionalPropertiesSchema
additionalPropertiesAllowed:(BOOL)additionalPropertiesAllowed
patternBasedPropertySchemas:(NSDictionary *)patternBasedPropertySchemas {
  NSAssert(additionalPropertiesSchema == nil || additionalPropertiesAllowed,
           @"Cannot have additional properties schema if additional properties "
           @"are not allowed.");
...
- (NSString *)description {
  ...
  if (self.additionalPropertiesSchema != nil) {
    additionalPropertiesDescription =
        self.additionalPropertiesSchema.description;
  } else {
    additionalPropertiesDescription =
        (self.additionalPropertiesAllowed ? @"allowed" : @"not allowed");
  }

  return [[super description]
      stringByAppendingFormat:@"{ %lu properties; additional properties: %@; "
                              @"%lu pattern properties }",
                              (unsigned long)self.propertySchemas.count,
                              additionalPropertiesDescription,
                              (unsigned long)
                                  self.patternBasedPropertySchemas.count];
}

+ (NSSet *)assignedKeywords {
  return [NSSet setWithArray:@[
    kSchemaKeywordProperties,
    kSchemaKeywordAdditionalProperties,
    kSchemaKeywordPatternProperties
  ]];
}

+ (instancetype)validatorWithDictionary:(NSDictionary *)schemaDictionary
                          schemaFactory:(VVJSONSchemaFactory *)schemaFactory
                                  error:(NSError *__autoreleasing *)error {
  id propertiesObject = schemaDictionary[kSchemaKeywordProperties];
  id additionalPropertiesObject =
      schemaDictionary[kSchemaKeywordAdditionalProperties];
  id patternPropertiesObject =
      schemaDictionary[kSchemaKeywordPatternProperties];
  ...
  if ([propertiesObject isKindOfClass:[NSDictionary class]]) {
    NSMutableDictionary *schemas =
        [NSMutableDictionary dictionaryWithCapacity:[propertiesObject count]];

    __block BOOL success = YES;
    __block NSError *internalError = nil;
    [propertiesObject enumerateKeysAndObjectsUsingBlock:^(
                          NSString *property, id schemaObject, BOOL *stop) {
      // schema object must be a dictionary
      if ([schemaObject isKindOfClass:[NSDictionary class]]) {
        // each schema will have scope extended by "/properties/#" where # is
        // property name
        VVJSONSchemaFactory *propertySchemaFactory =
            [schemaFactory factoryByAppendingScopeComponentsFromArray:@[
              kSchemaKeywordProperties,
              property
            ]];

        VVJSONSchema *propertySchema =
            [propertySchemaFactory schemaWithDictionary:schemaObject
                                                  error:&internalError];
        ...
        internalError = [NSError
            vv_JSONSchemaErrorWithCode:VVJSONSchemaErrorCodeInvalidSchemaFormat
                         failingObject:schemaObject];
      }
    ...
    if (error != NULL) {
      *error = [NSError
          vv_JSONSchemaErrorWithCode:VVJSONSchemaErrorCodeInvalidSchemaFormat
                       failingObject:schemaDictionary];
    }
    return nil;
  }
  ...
  if ([additionalPropertiesObject isKindOfClass:[NSDictionary class]]) {
    // parse as a schema object; schema will have scope extended by
    // "/additionalProperties"
    VVJSONSchemaFactory *additionalSchemaFactory = [schemaFactory
        factoryByAppendingScopeComponent:kSchemaKeywordAdditionalProperties];

    additionalPropertiesSchema =
        [additionalSchemaFactory schemaWithDictionary:additionalPropertiesObject
                                                error:error];
    if (additionalPropertiesSchema == nil) {
      return nil;
    }
  } else if ([additionalPropertiesObject isKindOfClass:[NSNumber class]] &&
             [additionalPropertiesObject vv_isBoolean]) {
    // parse as a boolean
  ...
  if ([patternPropertiesObject isKindOfClass:[NSDictionary class]]) {
    NSMutableDictionary *schemas = [NSMutableDictionary
        dictionaryWithCapacity:[patternPropertiesObject count]];

    __block BOOL success = YES;
    __block NSError *internalError = nil;
    [patternPropertiesObject enumerateKeysAndObjectsUsingBlock:^(
                                 NSString *pattern, id schemaObject,
                                 BOOL *stop) {
      // schema object must be a dictionary
      if ([schemaObject isKindOfClass:[NSDictionary class]]) {
        ...
        if (regexp != nil) {
          // each schema will have scope extended by "/patternProperties/#"
          ...
          success = NO;
          internalError =
              [NSError vv_JSONSchemaErrorWithCode:
                           VVJSONSchemaErrorCodeInvalidRegularExpression
                                    failingObject:pattern
...
- (BOOL)validateInstance:(id)instance
  ...
  __block BOOL success = YES;
  __block NSError *internalError = nil;
  [instance enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj,
                                                BOOL *stop) {
    // enumerate and validate all schemas applicable to the property
    BOOL enumerationSuccess = [self
        enumerateSchemasForProperty:key
                          withBlock:^(VVJSONSchema *schema, BOOL *innerStop) {
                            [context pushValidationPathComponent:key];
                            BOOL result =
                                [schema validateObject:obj
                                             inContext:context
                                                 error:&internalError];
    ...
    if (enumerationSuccess == NO) {
      NSString *failureReason = [NSString
          stringWithFormat:@"Additional property '%@' is not allowed.", key];
      internalError = [NSError
          vv_JSONSchemaValidationErrorWithFailingValidator:self
...
- (BOOL)enumerateSchemasForProperty:(NSString *)property
                          withBlock:(void (^)(VVJSONSchema *schema,
                                              BOOL *stop))block {
  ...
  [self.patternBasedPropertySchemas
      enumerateKeysAndObjectsUsingBlock:^(NSRegularExpression *regexp,
                                          VVJSONSchema *schema, BOOL *stop) {
        if ([regexp numberOfMatchesInString:property
                                    options:(NSMatchingOptions)0
                                      range:propertyFullRange] != 0) {
          visitedOnce = YES;
  ...
  if (visitedOnce == NO) {
    // if applicable schema was not found, respect additional properties
    // configuration:
    if (self.additionalPropertiesSchema != nil) {
      ...
      // additional properties schema is not defined, but any additional
