...
- (NSString *)description {
  NSString *optionsList = [[self.valueOptions valueForKey:@"description"]
      componentsJoinedByString:@", "];
  return [[super description]
      stringByAppendingFormat:@"{ allowed values: %@ }", optionsList];
}
...
+ (instancetype)validatorWithDictionary:(NSDictionary *)schemaDictionary
  ...
  if ([enumObject isKindOfClass:[NSArray class]]) {
    ...
    if ([enumObject count] != 0 &&
        [enumObject vv_containsDuplicateJSONItems] == NO) {
