...
- (NSString *)description {
  return [[super description]
      stringByAppendingFormat:
          @"{ maximum length: %@, minimum length: %lu, pattern: %@ }",
          (self.maximumLength != NSUIntegerMax ? @(self.maximumLength)
                                               : @"none"),
          (unsigned long)self.minimumLength, self.regularExpression.pattern];
}

+ (NSSet *)assignedKeywords {
  return [NSSet setWithArray:@[
    kSchemaKeywordMaxLength,
    kSchemaKeywordMinLength,
    kSchemaKeywordPattern
  ]];
}
...
+ (instancetype)validatorWithDictionary:(NSDictionary *)schemaDictionary
  ...
  NSUInteger maxLengthValue =
      (maxLength != nil ? [maxLength unsignedIntegerValue] : NSUIntegerMax);
  NSUInteger minLengthValue =
      (minLength != nil ? [minLength unsignedIntegerValue] : 0);
  ...
  return [[self alloc] initWithMaximumLength:maxLengthValue
...
+ (BOOL)validateSchemaFormat:(NSDictionary *)schemaDictionary {
  ...
  // maxLength must be a number and not a boolean, and must be greater than or
  // equal to zero
  if (maxLength != nil) {
    if ([maxLength isKindOfClass:[NSNumber class]] == NO ||
        [maxLength vv_isBoolean] ||
        [(NSNumber *)maxLength compare:@0] == NSOrderedAscending) {
      return NO;
    }
  }
  // minLength must be a number and not a boolean, and must be greater than or
  // equal to zero
  if (minLength != nil) {
    if ([minLength isKindOfClass:[NSNumber class]] == NO ||
        [minLength vv_isBoolean] ||
        [(NSNumber *)minLength compare:@0] == NSOrderedAscending) {
      return NO;
    }
  }
...
- (BOOL)validateInstance:(id)instance
  ...
  // retrieve actual string length by converting it to UTF32 representation and
  // calculating number of 4-byte charaters
  // (see http://www.objc.io/issue-9/unicode.html for details)
  NSUInteger realLength =
      [instance lengthOfBytesUsingEncoding:NSUTF32StringEncoding] / 4;
  // check maximum and minimum length
  if (realLength > self.maximumLength || realLength < self.minimumLength) {
    if (error != NULL) {
      NSString *failureReason =
          [NSString stringWithFormat:@"String is %lu characters long.",
  ...
  if (self.regularExpression != nil) {
    ...
    if ([self.regularExpression numberOfMatchesInString:instance
