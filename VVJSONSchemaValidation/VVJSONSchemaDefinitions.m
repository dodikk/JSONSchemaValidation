...
- (NSString *)description {
  return [[super description]
      stringByAppendingFormat:@"{ %lu subschemas }",
                              (unsigned long)_schemas.count];
}
...
+ (instancetype)validatorWithDictionary:(NSDictionary *)schemaDictionary
  ...
  [definitions enumerateKeysAndObjectsUsingBlock:^(
                   NSString *key, id schemaObject, BOOL *stop) {
    ...
    // each subschema has its resolution scope extended by
    // "definitions/[schema_name]"
    VVJSONSchemaFactory *definitionFactory =
        [schemaFactory factoryByAppendingScopeComponentsFromArray:@[
          kSchemaKeywordDefinitions,
          key
        ]];

    VVJSONSchema *schema =
        [definitionFactory schemaWithDictionary:schemaObject
                                          error:&internalError];
    if (schema != nil) {
...
- (BOOL)validateInstance:(id)__unused instance
               inContext:(VVJSONSchemaValidationContext *)__unused context
