...
- (instancetype)initWithSchemaDependencies:(NSDictionary *)schemaDependencies
                      propertyDependencies:
                          (NSDictionary *)propertyDependencies {
  self = [super init];
  if (self) {
...
- (NSString *)description {
  return [[super description]
      stringByAppendingFormat:
          @"{ %lu schema dependencies; %lu property dependencies }",
          (unsigned long)self.schemaDependencies.count,
          (unsigned long)self.propertyDependencies.count];
}
...
+ (instancetype)validatorWithDictionary:(NSDictionary *)schemaDictionary
  ...
  [dependencies enumerateKeysAndObjectsUsingBlock:^(
                    NSString *propertyName, id dependencyObject, BOOL *stop) {
    if ([dependencyObject isKindOfClass:[NSDictionary class]]) {
      // dependency object is a dictionary - parse it as a schema dependency;
      // schema will have scope extended by "/dependencies/#" where # is
      // dependent property name
      VVJSONSchemaFactory *dependencySchemaFactory =
          [schemaFactory factoryByAppendingScopeComponentsFromArray:@[
            kSchemaKeywordDependencies,
            propertyName
          ]];

      VVJSONSchema *dependencySchema =
          [dependencySchemaFactory schemaWithDictionary:dependencyObject
                                                  error:&internalError];
      ...
      if (dependentPropertiesSet.count == 0 ||
          dependentPropertiesSet.count != [dependencyObject count]) {
        *stop = YES;
        success = NO;
        return;
      }
  ...
  if (success) {
    return [[self alloc] initWithSchemaDependencies:schemaDependencies
...
#pragma clang diagnostic ignored "-Wgnu-conditional-omitted-operand"
      *error = internalError
                   ?: [NSError vv_JSONSchemaErrorWithCode:
                                   VVJSONSchemaErrorCodeInvalidSchemaFormat
                                            failingObject:schemaDictionary];
...
- (BOOL)validateInstance:(id)instance
  ...
  [self.propertyDependencies
      enumerateKeysAndObjectsUsingBlock:^(
          NSString *property, NSSet *dependingProperties, BOOL *stop) {
  ...
  if (success == NO) {
    if (error != NULL) {
      NSMutableSet *missingProperties =
          [self.propertyDependencies[failingProperty] mutableCopy];
      ...
      NSString *failureReason =
          [NSString stringWithFormat:@"Object is missing properties '%@' as "
                                     @"required by property '%@'.",
                                     missingPropertiesList, failingProperty];
      *error = [NSError
  ...
  [self.schemaDependencies
      enumerateKeysAndObjectsUsingBlock:^(NSString *property,
                                          VVJSONSchema *schema, BOOL *stop) {
        if ([propertyNames containsObject:property]) {
          if ([schema validateObject:instance
                           inContext:context
                               error:&internalError] == NO) {
            success = NO;
            *stop = YES;
          }
        }
