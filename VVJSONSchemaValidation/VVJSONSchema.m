...
static NSString *const kJSONSchemaDefaultString =
...
+ (NSSet *)supportedMetaschemaURIs {
  ...
  dispatch_once(&onceToken, ^{
    NSArray *uris = @[
      [NSURL URLWithString:kJSONSchemaDefaultString],
...
- (NSString *)description {
  return [[super description]
      stringByAppendingFormat:
          @"{ %@; '%@': '%@'; %lu validators; %lu subschemas }", self.uri,
          self.title, self.schemaDescription,
          (unsigned long)self.validators.count,
          (unsigned long)self.subschemas.count];
}

#pragma mark - Schema parsing

+ (instancetype)schemaWithDictionary:(NSDictionary *)schemaDictionary
                             baseURI:(NSURL *)baseURI
                    referenceStorage:(VVJSONSchemaStorage *)referenceStorage
                               error:(NSError *__autoreleasing *)error {
  // retrieve metaschema URI
  id metaschemaURIString = schemaDictionary[kSchemaKeywordSchema];
  if (metaschemaURIString != nil &&
      [metaschemaURIString isKindOfClass:[NSString class]] == NO) {
    if (error != NULL) {
      *error = [NSError
  ...
  if ([[self unsupportedMetaschemaURIs] containsObject:metaschemaURI]) {
    if (error != NULL) {
      *error = [NSError
          vv_JSONSchemaErrorWithCode:VVJSONSchemaErrorCodeIncompatibleMetaschema
                       failingObject:metaschemaURIString];
    }
    return nil;
  }

  // retrieve validator mapping for this metaschema
  NSDictionary *keywordsMapping =
      [self validatorsMappingForMetaschemaURI:metaschemaURI];
  ...
  // have to be careful around autorelease pool and reference-returned
  // autoreleasing objects...
  NSError *internalError = nil;
  @autoreleasepool {
    // instantiate a root schema factory and use it to create the schema
    VVJSONSchemaFactory *factory =
        [VVJSONSchemaFactory factoryWithScopeURI:baseURI
                                 keywordsMapping:keywordsMapping];
    schema =
        [factory schemaWithDictionary:schemaDictionary error:&internalError];
    ...
    if (schema != nil) {
      ...
      if (resolvingStorage != nil) {
        ...
        BOOL success =
            [schema resolveReferencesWithSchemaStorage:resolvingStorage
                                                 error:&internalError];
        ...
        // if creating a schema storage failed, it means there are duplicate
        // scope URIs
        internalError =
            [NSError vv_JSONSchemaErrorWithCode:
                         VVJSONSchemaErrorCodeDuplicateResolutionScope
                                  failingObject:schemaDictionary];
      }
    }
  }
...
+ (instancetype)schemaWithData:(NSData *)schemaData
                       baseURI:(NSURL *)baseURI
              referenceStorage:(VVJSONSchemaStorage *)referenceStorage
                         error:(NSError *__autoreleasing *)error {
  id object = [NSJSONSerialization JSONObjectWithData:schemaData
                                              options:(NSJSONReadingOptions)0
                                                error:error];

  if ([object isKindOfClass:[NSDictionary class]]) {
    return [self schemaWithDictionary:object
                              baseURI:baseURI
...
- (BOOL)resolveReferencesWithSchemaStorage:(VVJSONSchemaStorage *)schemaStorage
                                     error:(NSError *__autoreleasing *)error {
  ...
  [self visitUsingBlock:^(VVJSONSchema *subschema, BOOL *stop) {
    ...
    VVJSONSchemaReference *referenceSubschema =
        (VVJSONSchemaReference *)subschema;
    ...
    if (referencedSchema != nil) {
      ...
      internalError =
          [NSError vv_JSONSchemaErrorWithCode:
                       VVJSONSchemaErrorCodeUnresolvableSchemaReference
                                failingObject:referenceURI];
      *stop = YES;
    }
  }];
...
- (BOOL)detectReferenceCyclesWithError:(NSError *__autoreleasing *)error {
  ...
  [self visitUsingBlock:^(VVJSONSchema *subschema, BOOL *stop) {
    ...
    VVJSONSchemaReference *referencePointer =
        (VVJSONSchemaReference *)subschema;
    ...
    do {
      if ([referenceChain containsObject:referencePointer] == NO) {
        ...
        NSAssert(referencedSchema != nil,
                 @"Assuming all schema references are already resolved.");
        ...
        internalError = [NSError
            vv_JSONSchemaErrorWithCode:VVJSONSchemaErrorCodeReferenceCycle
                         failingObject:referencePointer];
        *stop = YES;
        return;
      }
...
- (BOOL)validateObject:(id)object
  ...
  BOOL success =
      [context pushValidatedSchema:self object:object withError:error];
  if (success == NO) {
    return NO;
  }

  for (id<VVJSONSchemaValidator> validator in self.validators) {
    if ([validator validateInstance:object inContext:context error:error] ==
        NO) {
      success = NO;
      break;
    }
...
- (BOOL)validateObjectWithData:(NSData *)data
                         error:(NSError *__autoreleasing *)error {
  id object =
...
// maps metaschema URIs to dictionaries which, in turn, map string keywords to
// validator classes
...
+ (NSDictionary *)validatorsMappingForMetaschemaURI:(NSURL *)metaschemaURI {
  ...
  // if not a standard supported supported metaschema URI, retrieve its custom
  // keywords
  NSDictionary *customKeywordsMapping = nil;
  if (metaschemaURI != nil &&
      [[self supportedMetaschemaURIs] containsObject:metaschemaURI] == NO) {
    customKeywordsMapping = schemaKeywordsMapping[metaschemaURI];
  }

  // retrieve keywords mapping for standard metaschema and extend it with custom
  // one if necessary
  NSDictionary *effectiveKeywordsMapping =
      schemaKeywordsMapping[[self defaultMetaschemaURI]];
  if (customKeywordsMapping.count > 0) {
    NSMutableDictionary *extendedMapping =
        [effectiveKeywordsMapping mutableCopy];
...
+ (BOOL)registerValidatorClass:(Class<VVJSONSchemaValidator>)validatorClass
              forMetaschemaURI:(NSURL *)metaschemaURI
  ...
  if (metaschemaURI == nil ||
      [[self supportedMetaschemaURIs] containsObject:metaschemaURI]) {
  ...
  if (keywords.count == 0) {
    if (error != NULL) {
      *error = [NSError vv_JSONSchemaErrorWithCode:
                            VVJSONSchemaErrorCodeNoValidatorKeywordsDefined
                                     failingObject:validatorClass];
    }
    return NO;
  }

  // check that the new validator does not define any keywords already defined
  // by another validator in the same scope
  NSDictionary *effectiveValidatorsMapping =
      [self validatorsMappingForMetaschemaURI:metaschemaURI];
  if ([[NSSet setWithArray:effectiveValidatorsMapping.allKeys]
          intersectsSet:keywords]) {
    if (error != NULL) {
      *error = [NSError vv_JSONSchemaErrorWithCode:
                            VVJSONSchemaErrorCodeValidatorKeywordAlreadyDefined
                                     failingObject:validatorClass];
    }
    return NO;
  }
...
#pragma clang diagnostic ignored "-Wgnu-conditional-omitted-operand"
  ...
  NSMutableDictionary *mapping =
      [schemaKeywordsMapping[metaschemaURI] mutableCopy]
